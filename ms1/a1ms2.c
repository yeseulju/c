...
//  Student ID    : 154673198
//  Student Email : yju9@myseneca.ca
//  Course Section: IPC 144 NEE
// #############################################################################################
...
int main(void)
{
    ...
    // !!! <STUDENT CODE HERE> !!!
        ...
        person.year = getIntFromRange(currentYear() - 110, currentYear() - 18);
...
void displayAccount(const struct Account account, const struct UserLogin login, const struct Person person)
{
   ...
   printf("%05d %-9s %-15s %5d %11.2lf %-10s %-10s %8s\n", account.number, account.type == 'c' ? "CUSTOMER" : "AGENT", person.name, person.year, person.income, person.country, login.login, login.password);
     printf("\n");
}
